...
import std.ascii;   // Changed from std.ctype to std.ascii for D2
...
struct Text
{
    ...
    int TTinr()
    {
        ...
        return std.ascii.toUpper(cast(char)c);  // Changed for D2
    }
    ...
    {   char[100] buffer;
        ...
        va_list ap;
        va_start(ap, format);
        count = vsnprintf(buffer.ptr, buffer.length, format, ap);
        va_end(ap);
        smes(buffer.ptr);
    }
    ...
    int DS(int row)
    {
      if (narrow)           // if 40 column display
            ...
            return (row << 8) + 46;  // Start messages at col 46 to leave room for 11 players
    }
